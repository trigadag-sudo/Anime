...
export default function AnimePlayer({ shikimoriId, title }: AnimePlayerProps) {
  ...
  const getNextProviderIndex = (current: number, blocked: number[]) => {
    return providerPool.findIndex((_, idx) => idx > current && !blocked.includes(idx));
  };

  const handleSourceFallback = (reason: string) => {
    setFailedProviders((prev) => {
      const nextFailed = prev.includes(activeProviderIndex) ? prev : [...prev, activeProviderIndex];
      const nextIndex = getNextProviderIndex(activeProviderIndex, nextFailed);
  ...
  }, [activeProviderIndex, mounted, allFailed]);
  ...
  }, [activeProvider, mounted, allFailed]);
